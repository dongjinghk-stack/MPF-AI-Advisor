...
const EnrollmentForm: React.FC<EnrollmentFormProps> = ({ prefillAllocation }) => {
  ...
  useEffect(() => {
    // Load funds for allocation lists, focusing on the target scheme (e.g. MASS/YF Life as per prompt context, 
    // but using generic logic to show all or filtered). 
    // For this form, we'll display a subset or the ones relevant to the "Transfer-In" scheme.
    // Assuming the target is "MASS Mandatory Provident Fund Scheme" based on the HTML file context.
    const funds = getFunds().filter(f => f.scheme_name.includes('MASS') || f.mpf_trustee === 'YF Life');
    setAvailableFunds(funds.length > 0 ? funds : getFunds().slice(0, 10)); // Fallback if filter matches nothing
  }, []);

  useEffect(() => {
    if (prefillAllocation && availableFunds.length > 0) {
      // Map scenario allocations to the form's transfer allocations
      const newAllocations: Record<string, number> = {};
      prefillAllocation.allocations.forEach(alloc => {
        // Try to find matching fund in the available funds list (target scheme)
        // This simulates mapping "Recommended" funds to "Available Scheme" funds
        // In a real app, this mapping logic would be more complex.
        // For now, we'll try to match by name or just use the allocation if the fund exists in the target list.
      ...
      // If we couldn't match exactly (e.g. recommending Manulife funds but transferring INTO MASS scheme),
      // we might just pre-fill based on risk profile or leave empty. 
      // For this demo, let's pre-fill the 'Transfer Fund Allocation' with the scenario values 
      // assuming the user wants to mirror that strategy in the new scheme.
      // We will map by fuzzy name or risk class if exact match fails? 
      // Let's just set it directly if the names match, otherwise leave blank to prompt user.
  ...
  // --- Validation Logic (Ported from HTML) ---
